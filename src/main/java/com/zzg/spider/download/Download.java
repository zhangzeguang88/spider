package com.zzg.spider.download;
...
//		request.setProxyPort(port);
		...
		if (page != null) {
			...
			RedisUtil.lpush("sourceQueue", page.getWebResponse().getContentAsString());
			
			//将url source mac 哪台机器存入数据库
			CrawlHtml ct = new CrawlHtml();
			ct.setUrl(url);
			ct.setSource(page.getWebResponse().getContentAsString());
			ct.setAddr(Download.macAddr);//todo
			ct.setDate(DateTimeUtil.getAddDate("yyyy-MM-dd hh:mm:ss",0));
			crawlHtmlMapper.save(ct);
		}
	}
